...
#New Type of data needed for Random Forest Library
...
set.seed(1234)
rfregressor = randomForest(formula= factor(Severity_O1_1_Cash) ~.,
                           data = training, ntree=30, mtry=125,
                           importance = TRUE, type = "classification" )

# Predicting on train set
predTrain <- predict(rfregressor, training, type = "class")
# Checking classification accuracy
table(predTrain, training$Severity_O1_1_Cash)  

# Predicting on test set
predTest <- predict(rfregressor, test, type = "class")
mean(predTest == test$Severity_O1_1_Cash)                    
table(predTest,test$Severity_O1_1_Cash)  
